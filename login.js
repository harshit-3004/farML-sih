function login_validation(){
    ...
    if(email == ""){
        ...
        document.getElementById('loginbutton').appendChild = "<div class='modal fade' id='exampleModal' tabindex='-1' aria-labelledby='exampleModalLabel' aria-hidden='true'><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><h5 class='modal-title' id='exampleModalLabel'>Welcome back! Proceed with your crop.</h5><button type='button' class='btn-close' data-bs-dismiss='modal' aria-label='Close'></button></div><div class='modal-body'><button type='button' class='btn btn-primary' onclick='window.location.href='./index.html''>Registered Crop</button><br><button type='button' class='btn btn-primary' style='margin-top: 5px;' onclick='window.location.href='./crop registration.html''>New Crop</button></div></div></div></div>";
    }
}